...
// action creator
export const getPostDetail = createAction(GET_POST_DETAIL, adminApi.getPostDetail);
...
export default handleActions({
    ...
    ...pender({
        ...
        onSuccess: (state, action) => {
            ...
            return state.set('postDetail', post)
                        .set('tracks', trackArr)
    ...
    [PLAY_NEXT]: (state, action) => {
        ...
        if(current === length - 1) {
            ...
            return state.setIn(['player', 'current'], 0)
                        .setIn(['player', 'url'], url);
        }
        const tracks = state.get('tracks');
        const url = `/media/tracks/${tracks[parseInt(state.getIn(['player', 'current']), 10) + 1]}`;
        return state.setIn(['player', 'current'], parseInt(current, 10) + 1)
                    .setIn(['player', 'url'], url);
    },
    [PLAY_PREV]: (state, action) => {
        ...
        if(current === 0) {
            ...
            return state.setIn(['player', 'current'], length - 1)
                        .setIn(['player', 'url'], url);
        }
        const tracks = state.get('tracks');
        const url = `/media/tracks/${tracks[parseInt(state.getIn(['player', 'current']), 10) - 1]}`;
    ...
    [SET_PLAYING]: (state, action) => {
        return state.setIn(['player', 'playing'], true);
    },
    [SET_STOP]: (state, action) => {
        return state.setIn(['player', 'playing'], false);
    },
    [SELECT_SONG]: (state, action) => {
        const { index } = action.payload;
        const tracks = state.get('tracks');
        const url = `/media/tracks/${tracks[parseInt(index, 10)]}`;
        return state.setIn(['player', 'current'], parseInt(index, 10))
                    .setIn(['player', 'url'], url);
    }
}, initialState);