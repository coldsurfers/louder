        ...
        try {
            ...
            if (id) {
                if (will_delete_cover.size !== 0) {
                    await AdminActions.filterCover({ will_delete_cover: will_delete_cover.toJS()[0].split("/")[will_delete_cover.toJS()[0].split("/").length - 1] }, header);
                }
                formData.append('album_track_file_names', this.props.album_track_file_names.join(","));
            ...
            let album_track_file_names = [];
            uploaded_tracks.toJS().map(
                (track, i) => {
                    let url = track.url;
                    let splitted = url.split("/");
                    let lastUrl = splitted[splitted.length - 1];
...
export default connect(
    (state) => ({
        ...
        album_track_file_names: state.admin.getIn(['editor', 'album_track_file_names']),
