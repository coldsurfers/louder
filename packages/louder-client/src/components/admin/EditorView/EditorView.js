...
class EditorView extends Component {
  ...
  handleUpload = (e) => {
    const { onPost, id, cover_changed, album_title, artist_name, song_names } = this.props;
  ...
  render() {
    const { uploadedTracks, album_track_file_names,
      album_title, album_cover, artist_name, song_names, onResetInput, id } = this.props;
    const { handleFilterTrack, handleFilterUploaded, handleChange, handleChangeCover, handleUploadTrack
      , handleUpload } = this;
    const uploadedList = uploadedTracks.map(
      (track, i) => {
        return (
          <div key={track.id} id={track.id}>
            ...
            <span
              ...
              onClick={handleFilterTrack}>지우기</span>
          </div>
        )
      }
    );
    const uploaded_album_tracks = album_track_file_names.map(
      (track, i) => {
        return (
          <div id={i} key={i}>
            {track} <span id={i} style={{ cursor: "pointer" }} onClick={handleFilterUploaded}>지우기</span>
          </div>
        )
      }
    );
