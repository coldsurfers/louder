...
export const increaseCurrentTrackNumber = createAction(INCREASE_CURRENT_TRACK_NUMBER);
export const decreaseCurrentTrackNumber = createAction(DECREASE_CURRENT_TRACK_NUMBER);
...
export default handleActions({
    ...
    ...pender({
        ...
        onSuccess: (state, action) => {
            ...
            console.log(results);
            return state.setIn(['posts', 'count'], count)
    ...
    ...pender({
        ...
        onSuccess: (state, action) => {
            ...
            return state.set('uploaded_track_file_name', uploaded.track_file)
                .set('uploaded_tracks', state.get('uploaded_tracks').push({
                    id: uploaded.id,
                    url: uploaded.track_file
                }))
                .setIn(['editor', 'album_track_file_names'],
                    state.getIn(['editor', 'album_track_file_names']).concat(uploaded.track_file.split("/")[uploaded.track_file.split("/").length - 1]));

        }
    }),
    [FILTER_UPLOAD_TRACKS]: (state, action) => {
        const { id } = action.payload;
        const index = state.get('uploaded_tracks').toJS().findIndex((track, i) => {
            return track.id === parseInt(id, 10)
        });
        ...
        return state.set('will_delete_tracks', state.get('will_delete_tracks').push(willDeleteUrl))
            .update('uploaded_tracks', trackList => trackList.splice(parseInt(index, 10), 1));
    },
    ...pender({
    ...
    [INCREASE_CURRENT_TRACK_NUMBER]: (state, action) => {
        const current_track_number = state.getIn(['album', 'current_track_number']);
        return state.setIn(['album', 'current_track_number'], parseInt(current_track_number, 10) + 1);
    },
    ...
    [SET_TO_ZERO]: (state, action) => {
        return state.setIn(['album', 'current_track_number'], 0);
    },
    ...
    [FILTER_UPLOADED_TRACKS]: (state, action) => {
        ...
        editorTracks = editorTracks.filter(
            (track, i) => {
        ...
        return state.setIn(['editor', 'album_track_file_names'], editorTracks)
            .update('will_delete_tracks', value => value.concat(willDeleteTracks));
    },
    [COVER_CHANGED_EDITOR]: (state, action) => {

        return state.setIn(['editor', 'cover_changed'], true)
            .setIn(['editor', 'will_delete_cover'],
                state.getIn(['editor', 'will_delete_cover']).push(state.getIn(['editor', 'album_cover'])));
    },
    [UNMOUNT_POST]: (state, action) => {
        return state.set('album', Map({
            album_tracks: List(),
            album_track_length: null,
            current_track_number: null,
            last_track_number: null,
    ...
    ...pender({
        ...
        onSuccess: (state, action) => {
            ...
            return state.set('userList', fromJS(results))
